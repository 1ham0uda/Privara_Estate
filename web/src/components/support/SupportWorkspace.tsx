      ...
      <div className="space-y-6">
        <Card className="bg-white border-none shadow-sm" hover={false}>
          <div className={`flex items-center gap-3 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div>
              <h2 className="font-bold text-ink">{t('support.create_title')}</h2>
              <p className="text-sm text-brand-slate">{t('support.create_description')}</p>
            </div>
          </div>
        ...
        <Card className="p-0 bg-white border-none shadow-sm overflow-hidden" hover={false}>
          <div className="p-4 border-b border-soft-blue">
            <h2 className="font-bold text-ink">{t('support.your_tickets')}</h2>
          </div>
            ...
            <div className="max-h-[540px] overflow-y-auto">
              {tickets.map((ticket) => (
                <button
                  ...
                  className={`w-full p-4 border-b border-soft-blue text-left transition-colors hover:bg-cloud ${selectedId === ticket.id ? 'bg-blue-50' : ''} ${isRTL ? 'text-right' : ''}`}
                >
                  <div className={`flex items-center justify-between gap-2 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <span className="text-[11px] text-brand-slate">{formatDate(ticket.updatedAt || ticket.createdAt, language)}</span>
                  </div>
                  <p className="text-sm font-medium text-ink line-clamp-2">{ticket.text}</p>
                  <p className="text-xs text-brand-slate mt-1">{ticket.replies?.length || 0} {t('support.replies_count')}</p>
                </button>
              ))}
            </div>
          )}
            ...
            <div className={`p-5 border-b border-soft-blue flex items-center justify-between gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div>
                <div className={`flex items-center gap-2 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <h2 className="font-bold text-ink">{t('support.ticket_details')}</h2>
                  <Badge variant={selectedTicket.status === 'open' ? 'warning' : 'success'}>{t(`support.${selectedTicket.status}`)}</Badge>
                </div>
                <p className="text-sm text-brand-slate">{formatDate(selectedTicket.createdAt, language)}</p>
              </div>
            </div>
              ...
              {selectedTicket.replies?.map((reply, index) => {
                ...
                return (
                  <div key={`${selectedTicket.id}-reply-${index}`} className={`flex flex-col ${isOwnReply ? (isRTL ? 'items-start' : 'items-end') : (isRTL ? 'items-end' : 'items-start')}`}>
                    <div className={`max-w-[85%] rounded-2xl p-4 shadow-sm ${isOwnReply ? 'bg-ink text-white' : 'bg-white border border-soft-blue text-ink'}`}>
                      <div className={`flex items-center gap-2 mb-2 text-xs ${isOwnReply ? 'text-white/70' : 'text-brand-slate'} ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <span className={`font-semibold ${isOwnReply ? 'text-white' : 'text-ink'}`}>{reply.senderName}</span>
                        <span>•</span>
                      ...
                      <p className={`text-[11px] mt-2 ${isOwnReply ? 'text-brand-slate' : 'text-brand-slate'}`}>{formatDate(reply.createdAt, language)}</p>
                    </div>
                  </div>
                );
              })}
              ...
              <form onSubmit={handleSendReply} className="p-5 border-t border-soft-blue bg-white">
                <div className="space-y-3">
                  <textarea
                    ...
                    className={`w-full h-24 px-4 py-3 bg-cloud border border-soft-blue rounded-xl focus:border-blue-600 focus:outline-none text-sm resize-none ${isRTL ? 'text-right' : ''}`}
                  />
