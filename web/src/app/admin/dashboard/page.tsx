...
export default function AdminDashboard() {
  ...
  const chartData = [
    { name: t('admin.dashboard.stat.unassigned'), value: unassignedCases.length, color: '#f59e0b' },
    { name: t('admin.dashboard.stat.active'), value: activeCases.length, color: '#3b82f6' },
    { name: t('admin.dashboard.stat.completed'), value: completedCases.length, color: '#10b981' },
  ];
                    ...
                    {consultants.map(con => {
                      ...
                      return (
                        <div key={con.uid} className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                          ...
                          <Badge variant={load > 3 ? 'warning' : 'success'} className="text-[10px]">{load} {t('admin.dashboard.stat.active_cases')}</Badge>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    {qualitySpecialists.map(q => {
                      ...
                      return (
                        <div key={q.uid} className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                          ...
                          <Badge variant={load > 5 ? 'warning' : 'success'} className="text-[10px]">{load} {t('admin.dashboard.stat.active_cases')}</Badge>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="fixed inset-0 bg-ink/50 flex items-center justify-center z-50 p-4" onClick={(e) => { if (e.target === e.currentTarget) setShowStaffDetailsModal(false); }}>
          ...
          <Card className={`w-full p-8 bg-white border-none shadow-2xl ${isRTL ? 'text-right' : ''}`} hover={false}>
            ...
            <div className={`flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <Button 
                variant={selectedStaff.status === 'deactivated' ? 'primary' : 'outline'}
