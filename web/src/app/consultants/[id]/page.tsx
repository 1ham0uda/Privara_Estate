  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className={`max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 ${isRTL ? 'rtl' : 'ltr'}`}>
        <button onClick={() => router.back()} className={`flex items-center text-gray-500 hover:text-black mb-8 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div className="space-y-8">
          ...
          <Card className="p-8 border-none shadow-sm" hover={false}>
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="w-32 h-32 bg-gray-100 rounded-3xl overflow-hidden relative">
                {profile.avatarUrl ? (
                  <Image 
              ...
              <div className={`flex-1 text-center ${isRTL ? 'md:text-right' : 'md:text-left'}`}>
                ...
                <p className="text-gray-500 mb-4">{profile.experienceYears} {t('consultant.years_exp_text')}</p>
            ...
            <div className="md:col-span-2 space-y-8">
              <section>
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <Briefcase className="w-5 h-5 text-gray-400" /> {t('consultant.about')}
                </h2>
              ...
              <section>
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <Award className="w-5 h-5 text-gray-400" /> {t('consultant.prof_summary')}
                </h2>
                <Card className="p-6 border-none shadow-sm" hover={false}>
              ...
              <section>
                ...
                <Card className="p-6 border-none shadow-sm" hover={false}>
                  <div className="space-y-4">
                    <button className={`w-full flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <Linkedin className="w-5 h-5 text-blue-600" />
                        <span className="text-sm font-medium">{t('consultant.linkedin')}</span>
                      </div>
                      <Globe className={`w-4 h-4 text-gray-300 ${isRTL ? 'rotate-180' : ''}`} />
                    </button>
